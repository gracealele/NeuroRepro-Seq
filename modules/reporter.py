...
def write_report(
    ...
    # Header
    lines += [
        sep, "  PPP Molecular Subtype Pipeline v2.0  –  Analysis Report",
        sep, ""]
    ...
    lines += [
        ...
        f" Normalization : {cfg.normalization.upper()}",
        f" Variance filter : MAD (top {cfg.top_var_genes} genes)",
        f" Dim. reduction : {cfg.method} ({'Ledoit-Wolf' if cfg.ledoit_wolf_shrinkage else 'standard'})",
        f" Components : {cfg.n_components}",
        f" Clustering : Consensus k-means ({cfg.n_iterations} iters, sub-rate={cfg.subsample_rate:.0%})",
        f"sub-rate={cfg.subsample_rate:.0%})",
    ...
    for st, hits in pathway_rep.items():
        ...
        if hits:
            for gs_name, genes, pathway_hints in hits:
                lines.append(f"    [{gs_name}]  overlapping genes: {', '.join(genes[:6])}")
