...
from modules.visualisation import (plot_consensus_heatmaps, plot_embedding, plot_marker_heatmap, 
...
def run(cfg: PipelineConfig) -> dict:
    ...
    with profiler("8_visualisation"):
        plot_consensus_heatmaps(matrices, optimal_k, out / "consensus_heatmaps.png", cfg)
        plot_embedding(embed, subtypes, true_labels, out / "embedding.png", cfg)
        plot_marker_heatmap(expr, subtypes, markers, out / "marker_heatmap.png", cfg)
        plot_k_selection(metrics, optimal_k, out / "k_selection.png", cfg)
        plot_geneset_scores(enrichment, out / "geneset_enrichment.png", cfg)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default=None, help="Path to a JSON config file (optional)")
    parser.add_argument("--geo", default=None, help="GEO accession ID, e.g. GSE152795 (overrides config.use_geo)")
    parser.add_argument("--out", default=None, help="Output directory (overrides config.out_dir)")
    parser.add_argument("--n-samples", type=int, default=None, help="Number of synthetic samples (overrides config)")
    parser.add_argument("--k-max", type=int, default=None, help="Maximum k to test (overrides config)")
