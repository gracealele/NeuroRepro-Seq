...
class TestGenesets(unittest.TestCase):
    ...
    def test_overlap(self):
        ...
        self.assertIn("Neuroinflammatory", r); 
        self.assertIn("IL6", r["Neuroinflammatory"])
...
class TestDataLoader(unittest.TestCase):
    def test_shape(self):
        self.assertEqual(_RAW.shape, (_CFG.synthetic_n_genes, 
                         _CFG.synthetic_n_samples))
        
    def test_label_length(self):
        self.assertEqual(len(_TLBLS), _CFG.synthetic_n_samples)
    
    def test_no_negatives(self):    
        self.assertTrue((_RAW.values >= 0).all())
    
    def test_label_names_valid(self):
        for l in _TLBLS: 
            self.assertIn(l, PPP_GENESETS)
...
class TestDimReduction(unittest.TestCase):
    ...
    def test_no_nan(self):     
        self.assertFalse(np.isnan(_COORDS).any())
...
class TestClustering(unittest.TestCase):
    ...
    def test_optimal_k_in_range(self):  
        self.assertIn(_OPT_K, _CFG.k_range)
    ...
    def test_subtypes_length(self):     
        self.assertEqual(len(_SUBS), _CFG.synthetic_n_samples)
...
class TestCharacterisation(unittest.TestCase):
    ...
    def test_enrichment_cols(self):
        for st in _SUBS.unique(): 
            self.assertIn(st, _ENRICH.columns)

    def test_pathway_report_keys(self):
        for st in _SUBS.unique():
            self.assertIn(st, _PW); 
            self.assertIsInstance(_PW[st], list)
...
class TestVisualisation(unittest.TestCase):
    ...
    def test_consensus(self):
        p = self._p("v_ch.png")
        plot_consensus_heatmaps(_MATS, _OPT_K, p, _CFG); 
        self._ok(p)

    def test_embedding(self):
        p = self._p("v_emb.png")
        plot_embedding(_EMBED, _SUBS, None, p, _CFG); 
        self._ok(p)

    def test_embedding_true_labels(self):
        p = self._p("v_embt.png")
        plot_embedding(_EMBED, _SUBS, _TLBLS, p, _CFG); 
        self._ok(p)

    def test_marker_heatmap(self):
        p = self._p("v_mh.png")
        plot_marker_heatmap(_PROC, _SUBS, _MARKS, p, _CFG); 
        self._ok(p)

    def test_k_selection(self):
        p = self._p("v_ks.png")
        plot_k_selection(_MET, _OPT_K, p, _CFG); 
        self._ok(p)

    def test_geneset_scores(self):
        p = self._p("v_gs.png")
        plot_geneset_scores(_ENRICH, p, _CFG); 
        self._ok(p)

    def test_compute_profile(self):
        p = self._p("v_cp.png")
        df = pd.DataFrame({"stage":["a","b"],"time_s":[1.,2.],"peak_ram_mb":[50.,80.]})
        plot_compute_profile(df, p, _CFG); 
        self._ok(p)
...
class TestReporter(unittest.TestCase):
    ...
    def test_optimal_k_present(self):
        _, t = self._run("k"); 
        self.assertIn(f"Optimal k = {_OPT_K}", t)
